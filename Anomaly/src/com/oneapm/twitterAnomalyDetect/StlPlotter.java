    ...
    private JFreeChart createChart() {
      ...
      try {
        ...
        for (int i = 0; i < series.length; i++) {
          ...
          seriests.addOrUpdate((RegularTimePeriod) cons.newInstance(d), series[i]);
          seasonalts.addOrUpdate(new Minute(d), seasonal[i]);
          trendts.addOrUpdate(new Minute(d), trend[i]);
          remainderts.addOrUpdate(new Minute(d), remainder[i]);
        }
